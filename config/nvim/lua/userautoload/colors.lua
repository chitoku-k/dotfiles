vim.opt.background = 'dark'

if vim.env.TERM ~= 'linux' then
  vim.opt.termguicolors = true
end

if not pcall(vim.cmd.colorscheme, 'base16-ocean') then
  vim.cmd.colorscheme('desert')
  return
end

if vim.fn.has('ttyout') == 1 then
  vim.api.nvim_set_hl(0, 'Normal', {})
  vim.api.nvim_set_hl(0, 'NormalNC', {})
end

local colors = require('base16-colorscheme').colorschemes.ocean
vim.api.nvim_set_hl(0, 'CocErrorFloat', { fg = colors.base08 })
vim.api.nvim_set_hl(0, 'CocErrorHighlight', { sp = colors.base08, undercurl = true })
vim.api.nvim_set_hl(0, 'CocErrorSign', { fg = colors.base05, bg = colors.base08 })
vim.api.nvim_set_hl(0, 'CocHintFloat', { link = 'CocInfoFloat' })
vim.api.nvim_set_hl(0, 'CocHintHighlight', { link = 'CocInfoHighlight' })
vim.api.nvim_set_hl(0, 'CocHintSign', {})
vim.api.nvim_set_hl(0, 'CocHoverRange', { link = 'CursorColumn' })
vim.api.nvim_set_hl(0, 'CocInfoFloat', { fg = colors.base0D })
vim.api.nvim_set_hl(0, 'CocInfoHighlight', { sp = colors.base0D, undercurl = true })
vim.api.nvim_set_hl(0, 'CocInfoSign', { fg = colors.base01, bg = colors.base0D })
vim.api.nvim_set_hl(0, 'CocInlayHint', { link = 'Comment' })
vim.api.nvim_set_hl(0, 'CocMenuSel', { bg = colors.base02 })
vim.api.nvim_set_hl(0, 'CocNotificationProgress', { fg = colors.base0D })
vim.api.nvim_set_hl(0, 'CocSearch', { fg = colors.base0D })
vim.api.nvim_set_hl(0, 'CocSemAsyncKeyword', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'CocSemComment', {})
vim.api.nvim_set_hl(0, 'CocSemConstantVariable', { link = 'Boolean' })
vim.api.nvim_set_hl(0, 'CocSemDerive', { link = 'Type' })
vim.api.nvim_set_hl(0, 'CocSemEnumMember', { link = 'Boolean' })
vim.api.nvim_set_hl(0, 'CocSemKeyword', {})
vim.api.nvim_set_hl(0, 'CocSemLocalVariable', {})
vim.api.nvim_set_hl(0, 'CocSemMacro', { link = 'Macro' })
vim.api.nvim_set_hl(0, 'CocSemMacroBang', { link = 'Macro' })
vim.api.nvim_set_hl(0, 'CocSemNamespace', { italic = true })
vim.api.nvim_set_hl(0, 'CocSemNumber', {})
vim.api.nvim_set_hl(0, 'CocSemReadonly', { link = 'Boolean' })
vim.api.nvim_set_hl(0, 'CocSemReadonlyFunction', { link = 'Function' })
vim.api.nvim_set_hl(0, 'CocSemReadonlyProperty', {})
vim.api.nvim_set_hl(0, 'CocSemSelfTypeKeyword', { link = 'Special' })
vim.api.nvim_set_hl(0, 'CocSemString', {})
vim.api.nvim_set_hl(0, 'CocSemStruct', { link = 'Structure' })
vim.api.nvim_set_hl(0, 'CocSemTypeAlias', { link = 'Special' })
vim.api.nvim_set_hl(0, 'CocWarningFloat', { fg = colors.base0A })
vim.api.nvim_set_hl(0, 'CocWarningHighlight', { sp = colors.base0A, undercurl = true })
vim.api.nvim_set_hl(0, 'CocWarningSign', { fg = colors.base08, bg = colors.base0A })
vim.api.nvim_set_hl(0, 'Comment', { fg = colors.base03, italic = true })
vim.api.nvim_set_hl(0, 'Conceal', { fg = 'Grey' })
vim.api.nvim_set_hl(0, 'ConflictMarkerBegin', { fg = colors.base01, bg = colors.base0B })
vim.api.nvim_set_hl(0, 'ConflictMarkerEnd', { fg = colors.base01, bg = colors.base0D })
vim.api.nvim_set_hl(0, 'ConflictMarkerSeparator', { fg = colors.base01, bg = colors.base0C })
vim.api.nvim_set_hl(0, 'DiffAdd', { bg = '#334539' })
vim.api.nvim_set_hl(0, 'DiffChange', { bg = '#454133' })
vim.api.nvim_set_hl(0, 'DiffDelete', { bg = '#45333a' })
vim.api.nvim_set_hl(0, 'DiffLine', { bg = colors.base0D })
vim.api.nvim_set_hl(0, 'DiffText', { bg = '#5f5d42' })
vim.api.nvim_set_hl(0, 'EndOfBuffer', { fg = colors.base01 })
vim.api.nvim_set_hl(0, 'Error', { fg = colors.base00, bg = colors.base08 })
vim.api.nvim_set_hl(0, 'Flashy', { bg = colors.base01 })
vim.api.nvim_set_hl(0, 'FloatBorder', { link = 'WinSeparator' })
vim.api.nvim_set_hl(0, 'FoldColumn', { fg = colors.base0C, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'Identifier', { fg = colors.base05 })
vim.api.nvim_set_hl(0, 'LineNr', { fg = colors.base03, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'LspCxxHlGroupEnumConstant', { link = 'CocSemEnumMember' })
vim.api.nvim_set_hl(0, 'LspCxxHlGroupMemberVariable', { link = 'Normal' })
vim.api.nvim_set_hl(0, 'LspCxxHlGroupNamespace', { link = 'CocSemNamespace' })
vim.api.nvim_set_hl(0, 'LspCxxHlSymField', {})
vim.api.nvim_set_hl(0, 'LspCxxHlSymUnknown', {})
vim.api.nvim_set_hl(0, 'LspCxxHlSymVariable', {})
vim.api.nvim_set_hl(0, 'NormalFloat', { link = 'Pmenu' })
vim.api.nvim_set_hl(0, 'SignatureMarkerText', { fg = colors.base08, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'SignatureMarkText', { fg = colors.base0B, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'SignColumn', { fg = colors.base03, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'SpellBad', { fg = colors.base08, sp = colors.base08, undercurl = true })
vim.api.nvim_set_hl(0, 'StatusLine', { fg = colors.base04, bg = colors.base02 })
vim.api.nvim_set_hl(0, 'StatusLineNC', { fg = colors.base03, bg = colors.base01 })
vim.api.nvim_set_hl(0, 'TermCursor', { reverse = true })
vim.api.nvim_set_hl(0, 'TermCursorNC', {})
vim.api.nvim_set_hl(0, 'VertSplit', { fg = colors.base02, bg = colors.base02 })
vim.api.nvim_set_hl(0, 'WinBar', { bold = true })
vim.api.nvim_set_hl(0, 'WinBarNC', { link = 'WinBar' })

vim.api.nvim_set_hl(0, 'dockerfileFrom', { link = 'Tag' })
vim.api.nvim_set_hl(0, 'goGenerate', { link = 'Special' })
vim.api.nvim_set_hl(0, 'goGenerateVariables', { link = 'PreProc' })
vim.api.nvim_set_hl(0, 'goLabel', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'goRepeat', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'goType', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'helpHyperTextJump', { link = 'Underlined' })
vim.api.nvim_set_hl(0, 'javaScriptComment', { link = 'Comment' })
vim.api.nvim_set_hl(0, 'phpIdentifier', { link = 'Character' })
vim.api.nvim_set_hl(0, 'phpMethodsVar', { link = 'Function' })
vim.api.nvim_set_hl(0, 'phpRepeat', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'purescriptOperator', { link = 'Delimiter' })
vim.api.nvim_set_hl(0, 'qfError', { fg = colors.base08 })
vim.api.nvim_set_hl(0, 'sshconfigEnum', { link = 'Boolean' })
vim.api.nvim_set_hl(0, 'tomlKey', { link = 'Macro' })
vim.api.nvim_set_hl(0, 'typescriptEndColons', { link = 'jsObjectSeparator' })
vim.api.nvim_set_hl(0, 'typescriptIdentifier', { link = 'Constant' })
vim.api.nvim_set_hl(0, 'typescriptOperator', { link = 'Function' })
vim.api.nvim_set_hl(0, 'typescriptOpSymbols', { link = 'Function' })
vim.api.nvim_set_hl(0, 'typescriptReserved', { link = 'Keyword' })
vim.api.nvim_set_hl(0, 'yamlBlockMappingKey', { link = 'Macro' })
vim.api.nvim_set_hl(0, 'yamlFlowMappingKey', { link = 'Macro' })
vim.api.nvim_set_hl(0, 'yamlPlainScalar', { link = 'String' })
